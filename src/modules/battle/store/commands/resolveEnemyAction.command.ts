...
export type StoreSet = (fn: (state: BattleState) => Partial<BattleState>) => void;
...
export const resolveEnemyActions = async (get: StoreGet, set: StoreSet) => {
    ...
    for (const currentMonster of initialMonsters) {
        ...
        // --- BRANCH: ATTACK ---
        const attackResult = handleMoveAndAttack(state, freshMonster, plannedAttack);
        if (attackResult) {
            set((prev) => ({
                ...prev,
    ...
    const nextEnemyIntents = calculateAllIntents(finalState.heroes, finalState.monsters);
