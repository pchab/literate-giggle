...
import { resolveMoveEffect, resolvePushEffect, resolveStandardEffect, resolveSummonEffect } from "../../helpers/effect.resolvers";

export function resolveCard(anchorTargetId: AnchorTarget | null): BattleStoreServerAction {
    return ({ activeCard, heroes, monsters, summons, usedCardsThisTurn, usedMovesThisTurn, xpEarned, ...state }) => {
        ...
        card.effects.forEach((effect) => {
            switch (effect.type) {
                case "move":
                    draftHeroes = resolveMoveEffect(effect, anchorTargetId, heroId, draftHeroes);
                    break;
                case "summon":
                    draftSummons = resolveSummonEffect(effect, anchorTargetId, draftSummons);
                    break;
                case "push":
                    const pushResult = resolvePushEffect(effect, anchorTargetId, heroId, draftHeroes, draftMonsters, draftSummons);
                    draftHeroes = pushResult.heroes;
                    draftMonsters = pushResult.monsters;
                    break;
                default:
                    const stdResult = resolveStandardEffect(effect, anchorTargetId, heroId, draftHeroes, draftMonsters, vfx);
        ...
        const xpEarnedThisTurn = deadMonsters.reduce((acc, m) => acc + m.xpReward, 0);
