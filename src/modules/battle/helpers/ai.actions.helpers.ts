...
export function handleSummon(state: BattleState, caster: Monster, attack: Attack): Monster[] {
    ...
    const spawnPos = possibleSpawns.find((pos) => {
        const isOccupiedByHero = state.heroes.some((h) => h.gridPosition.col === pos.col && h.gridPosition.row === pos.row);
        const isOccupiedByMonster = state.monsters.some((m) => m.gridPosition.col === pos.col && m.gridPosition.row === pos.row);
...
export function handleMoveAndAttack(state: BattleState, attacker: Monster, attack: Attack) {
    const { reachableTarget, moveDest } = getIdealTarget(attacker, attack, state.heroes, state.monsters);
    ...
    const distanceToTarget = getManhattanDistance(moveDest, reachableTarget.gridPosition);
    ...
    // 2. Resolve Combat
    const collision = getActualTarget(moveDest, reachableTarget.gridPosition, state.heroes, nextMonsters);
    const finalTargetPos = collision ? collision.unit.gridPosition : reachableTarget.gridPosition;
