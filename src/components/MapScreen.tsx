...
import { type MapNode, type NodeType, useWorldStore } from '../store/world.store';

// Simple helper to get icons/colors for node types
const getNodeVisuals = (type: NodeType) => {
  switch (type) {
    case 'battle': return { color: 'bg-red-900 border-red-500', icon: '⚔️' };
    case 'elite': return { color: 'bg-purple-900 border-purple-500', icon: '💀' };
    case 'camp': return { color: 'bg-green-900 border-green-500', icon: '🔥' };
    case 'boss': return { color: 'bg-red-950 border-red-600 outline outline-4 outline-red-900', icon: '👑' };
    case 'event': return { color: 'bg-blue-900 border-blue-500', icon: '?' };
    default: return { color: 'bg-gray-800 border-gray-500', icon: '•' };
  }
};
...
export default function MapScreen() {
  ...
  const { currentTierIndex, currentNodeData }: { currentTierIndex: number; currentNodeData: MapNode } = useMemo(() => {
    let tierIndex = 0;
          ...
          return (
            <div key={tier} className="flex flex-col gap-8 relative">
              {nodes.map((node) => {
                ...
                const isClickable = isNextTier && currentNodeData?.nextNodes.includes(node.id);
                ...
                return (
                  <button
                    ...
                    `}
                  >
                    <span className="text-3xl mb-1 drop-shadow-md">{visuals.icon}</span>
